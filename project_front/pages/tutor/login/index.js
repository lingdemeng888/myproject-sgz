...
Page({
  ...
  sendCode(){ if(!this.data.id) return wx.showToast({ title:'先填写工号/手机号', icon:'none' });
    const c=genCode(); this.setData({ sentCode:c }); wx.showToast({ title:`验证码已发送到 ${maskPhone(this.data.id)}`, icon:'none' }); },
  login(){ if(!(this.data.id&&this.data.pwd)) return wx.showToast({ title:'请完善账号与密码', icon:'none' });
    if(this.data.code!==this.data.sentCode) return wx.showToast({ title:'验证码不正确', icon:'none' });
    wx.showToast({ title:'登录成功（示例）', icon:'success' }); },
  toRegister(){ wx.navigateTo({ url: '/pages/tutor/register/index' }); }
});
